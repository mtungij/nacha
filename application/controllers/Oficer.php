...
// check old password is match
    ...
    public function penalt_today()
    {
     ...
     $this->load->view('oficer/penalt_payment',['income'=>$income,'detail_income'=>$detail_income,'total_receved'=>$total_receved,'customer'=>$customer,'empl_data'=>$empl_data]);

    }

    public function transanction_oficer(){
      $this->load->model('queries');
      $blanch_id = $this->session->userdata('blanch_id');
      $empl_id = $this->session->userdata('empl_id');
      $manager_data = $this->queries->get_manager_data($empl_id);
      ...
      //  print_r($empl_oficer );
      //           exit();
      $this->load->view('oficer/teler_oficer',['empl_oficer'=>$empl_oficer,'total_deposit'=>$total_deposit,'total_withdrawal'=>$total_withdrawal,'cash_account'=>$cash_account,'privillage'=>$privillage]);
  }
    ...
    public function print_faini($from, $to, $blanch_id)
    {
     ...
     public function update_deducted_income_balance($blanch_id,$remove_deduction){
      ...
      public function mobile_payment()
      {
        ...
        $cash_transaction = $this->queries->get_mobile_transaction_blanch($blanch_id);
      
    
    

        //        echo "<pre>";
        // print_r($cash_transaction);
        //            exit();

        $this->load->view('oficer/mobile_payment',['cash_transaction' => $cash_transaction]);
     ...
     public function create_customer(){
      ...
      if ($check == TRUE) {
          ...
          if ($phoneExists) {
              ...
              $date = date("Y-m-d");
             $customer_id = $customer_id = $this->insert_customer_detail($comp_id,$blanch_id,$empl_id,$f_name,$m_name,$l_name,$gender,$date_birth,$age,$phone,$region_id,$district,$ward,$street,$date_reg,$registration_fee);;
...
public function modify_sponser($sp_id,$customer_id){
  ...
  public function loan_pending(){
		...
		$blanch_data = $this->queries->get_blanchData($blanch_id);
		$empl_data = $this->queries->get_employee_data($empl_id);
// $histor = $this->queries->get_loan_history($customer_id);
  ...
  $query = $this->db->query($sqldata);
   return true;
    ...
    public function view_aggrement($customer_id)
    {
        ...
        $html = $this->load->view('oficer/loan_aggrement', [
            "customer" => $customer,
            "mdhamini" => $mdhamini,
            "compdata" => $compdata,
...
public function insert_blanch_principal($comp_id,$blanch_id,$trans_id,$princ_status,$principal_insert){
 $this->db->query("INSERT INTO tbl_blanch_capital_principal (`comp_id`,`blanch_id`,`trans_id`,`princ_status`,`principal_balance`) VALUES ('$comp_id','$blanch_id','$trans_id','$princ_status','$principal_insert')");   
  ...
  return true;
    ...
    $this->load->view('oficer/expenses_requisition',['empl_data'=>$empl_data,'expenses'=>$expenses,'blanch_account'=>$blanch_account ,'request_exp'=>$request_exp,'expenses_total'=>$expenses_total]);
}
...
public function sendsms($phone,$massage){
  ...
  $token = "9|IjdYL2jzCfEfTHfGLfplX6pgoWsTloUCN0IUpwKTdb2aa7da";
